...
public class Controller : MonoBehaviour
{
    ...
    private float colorLerpDuration = 0.2f;
    ...
    void Start()
    {
        ...
        //Health Controller
        HealthInfo.Instance.UpdateMaxHealth(maxHealth);
        currentHealth = maxHealth;
        HealthInfo.Instance.UpdateCurrentHealth(currentHealth);
    ...
    void Update()
    {
        ...
        {
            if(OptionsMenu.Instance.isActiveAndEnabled)
            {
                OptionsMenu.Instance.Back();
            }
            else 
                PauseMenu.Instance.ReturnToGame(); //Nu pot iesi din pauza apasand ESC, dar pot da back cu ESC

        }
        if (CanPause && Input.GetButtonDown("Menu") && !OptionsMenu.Instance.isActiveAndEnabled) //Trebuie verificat sa nu fiu si in alt meniu, nu doresc sa dea overlap
        {
        ...
        if (isPulsating)
        {
            float targetIntensity = (currentIntensity == minIntensity) ? maxIntensity : minIntensity;
            currentIntensity = Mathf.MoveTowards(currentIntensity, targetIntensity, Time.deltaTime * pulsateSpeed);
            ...
            else
            {
                ...
                float t = Mathf.Clamp01(colorLerpTime / colorLerpDuration);
                vignette.color.Override(Color.Lerp(originalColor, targetColor, t));
            }
        }
    }
